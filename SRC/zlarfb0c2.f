...
*  ===========
*
*     SUBROUTINE ZLARFB0C2(C2JOB, APPLYT, SIDE, TRANS, DIRECT,
...
*     .. External Subroutines ..
      EXTERNAL          ZGEMM, ZTRMM, ZTRSM, XERBLA
*     ..
*     .. Local Scalars ..
...
*
         IF (TRANST) THEN
            IF (TSOLVE) THEN
               CALL ZTRSM('Right', 'Upper', 'Conjugate Transpose',
     ...
     $               'Non-unit', M, K, ONE, T, LDT, C, LDC)
         ...
         ELSE
            IF (TSOLVE) THEN
               CALL ZTRSM('Right', 'Lower', 'No Transpose',
     $               'Non-unit', M, K, ONE, T, LDT, C, LDC)
            ELSE
...
*
         IF (TSOLVE) THEN
            CALL ZTRSM('Left', 'Lower', 'No Transpose', 'Non-unit',
...
*
     ...
     $            'Non-unit', M, K, ONE, T, LDT, C(1, N-K+1), LDC)
            ELSE
               CALL ZTRMM('Right', 'Lower', 'Transpose',
     $            'Non-unit', M, K, ONE, T, LDT, C(1, N-K+1), LDC)
            END IF
         ELSE
            IF (TSOLVE) THEN
               CALL ZTRSM('Right', 'Upper', 'No Transpose',
     $            'Non-unit', M, K, ONE, T, LDT, C(1, N-K+1), LDC)
            ELSE
