...
*              C1 = v'*C2 = (C2'*v)'
*
               CALL ZGEMV('Conjugate Transpose', M-1, N, ONE,
     $              C(2,1), LDC, V, INCV, ZERO, C, LDC)
*
*              We must conjugate C1. and multply by TAU.
*              TODO: consider having a zgemv-like routine that allows for just conjugating x.
*
               DO J = 1, N
                  C(1,J) = -TAU*CONJG(C(1,J))
               END DO
*
*              C2 = C2 + v*C1
*
...
*              C1 = C2*v'
*
               CALL ZGEMV('No Transpose', M, N-1, ONE, C(1,2), LDC,
     $            V, INCV, ZERO, C, 1)
*
*              C1 = -tau*conjg(C1)
*
               DO I = 1, M
                  C(I,1) = -TAU*CONJG(C(I,1))
               END DO
*
*              C2 = C2 + C1*v
*
...
*              C1 = -tau*C2'*v
*
               CALL ZGEMV('Transpose', M-1, N, -TAU, C, LDC,
...
*              C1 = -tau*C2v'
*
               CALL ZGEMV('No Transpose', M, N-1, -TAU, C, LDC,
     $            V, INCV, ZERO, C(1,N), 1)
*
*              C2 = C2 + C1*v
*
